...
export default function MyPage() {
  ...
  useEffect(() => {
    ...
    const fetchRecentReads = async () => {
      try {
        ...
        setError('최근 읽은 소설을 불러오는데 실패했습니다.');
  ...
  const tabData = {
    recent: error ? getRecentlyRead() : getRecentlyRead(), // TODO: recentReads API 데이터 매핑
            ...
            {currentData.map((novel) => (
              <div key={novel.id} className="relative">
                ...
                {activeTab === 'recent' && novel.lastReadEpisode && (
                  <div className="absolute bottom-4 left-2 right-2 bg-black/70 text-white text-xs rounded p-2">
